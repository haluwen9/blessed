...
function Element(options) {
  ...
  this.on('focus', function () {
    if (this.type != 'textbox' || this.type != 'textarea' || this.type != 'checkbox' || this.type != 'radiobox') {
...
Element.prototype._wrapContent = function (content, width) {
  var tags = this.parseTags,
    ...
    out = [],
    no = 0,
    line, align, cap, total, i, part, j, lines, rest;
...
Element.prototype._getCoords = function (get, noscroll) {
  ...
  var xi = this._getLeft(get),
    ...
    el = this,
    fixed = this.fixed,
    coords, v, noleft, noright, notop, nobot, ppos, b;
...
Element.prototype.render = function () {
  ...
  var lines = this.screen.lines,
    ...
    yi = coords.yi,
    yl = coords.yl,
    x, y, cell, attr, ch, content = this._pcontent,
    ci = this._clines.ci[coords.base],
    battr, dattr, c, visible, i, bch = this.ch;
...
Element.prototype.deleteLine = function (i, n) {
  ...
  // if they're the same, or if they fit in the visible region entirely.
  var start = this._clines.length,
    diff, real = this._clines.ftor[i][0];
